...
{
    DEBUG_PRINT(Serial.printf("Tremolo::setParam(): setting param %d to %0.2f\n\r", paramIndex, paramValue));
    switch(paramIndex) {
...
void Tremolo::processMidi(int channel, int control, int value)
{
    ...
    if ((m_midiConfig[Bypass_e][MIDI_CHANNEL] == channel) && (m_midiConfig[Bypass_e][MIDI_CONTROL] == control)) {
        DEBUG_PRINT(Serial.println(String("BlackaddrAudio_Tremolo::Bypass: ") + val + String("%")));
        bypass(val);
        return;
    }

    if ((m_midiConfig[Waveform_e][MIDI_CHANNEL] == channel) && (m_midiConfig[Waveform_e][MIDI_CONTROL] == control)) {
        DEBUG_PRINT(Serial.println(String("BlackaddrAudio_Tremolo::Waveform: ") + val + String("%")));
        waveform(val);
        return;
    }

    if ((m_midiConfig[Rate_e][MIDI_CHANNEL] == channel) && (m_midiConfig[Rate_e][MIDI_CONTROL] == control)) {
        DEBUG_PRINT(Serial.println(String("BlackaddrAudio_Tremolo::Rate: ") + val + String("%")));
        rate(val);
        return;
    }

    if ((m_midiConfig[Depth_e][MIDI_CHANNEL] == channel) && (m_midiConfig[Depth_e][MIDI_CONTROL] == control)) {
        DEBUG_PRINT(Serial.println(String("BlackaddrAudio_Tremolo::Depth: ") + val + String("%")));
        depth(val);
        return;
    }

    if ((m_midiConfig[Volume_e][MIDI_CHANNEL] == channel) && (m_midiConfig[Volume_e][MIDI_CONTROL] == control)) {
        DEBUG_PRINT(Serial.println(String("BlackaddrAudio_Tremolo::Volume: ") + val + String("%")));
        volume(val);
        return;
    }
